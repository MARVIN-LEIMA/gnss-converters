...
START_TEST(test_pvt_failed_repair)
{
  ...
  calc_PVT(n_used, nms, false, true, ALL_CONSTELLATIONS, &soln, &dops,
           &raim_removed_sids);
...
START_TEST(test_pvt_repair)
{
  ...
  s8 code = calc_PVT(n_used, nms, false, true, ALL_CONSTELLATIONS, &soln, &dops,
                     &raim_removed_sids);
  fail_unless(code == 1,
...
START_TEST(test_pvt_vel_repair)
{
  ...
  s8 code = calc_PVT(n_used, nms, false, false, ALL_CONSTELLATIONS, &soln, &dops,
                     &raim_removed_sids);
  fail_unless(code == 1,
    "Return code should be 1 (pvt repair). Saw: %d\n", code);
  fail_unless(soln.n_sigs_used == n_used - 1,
...
START_TEST(test_pvt_raim_gps_l1ca_only)
{
  ...
  s8 code = calc_PVT(n_used, nms, false, false, GPS_L1CA_WHEN_POSSIBLE, &soln,
...
START_TEST(test_disable_pvt_raim)
{
  ...
  /* disable raim check */
  s8 code = calc_PVT(n_used, nms, true, true, ALL_CONSTELLATIONS, &soln, &dops,
                     &raim_removed_sids);
  fail_unless(code == 2,
...
START_TEST(test_count_sats)
{
  ...
  /* disable raim check */
  s8 code = calc_PVT(n_used, nms, true, false, ALL_CONSTELLATIONS, &soln, &dops,
                     &raim_removed_sids);
  fail_unless(code >= 0,
    "Return code should be >=0 (success). Saw: %d\n", code);
  fail_unless(soln.valid == 1,
...
START_TEST(test_count_sats_l1ca_only)
{
  ...
  /* disable raim check */
  s8 code = calc_PVT(n_used, nms, true, false, GPS_L1CA_WHEN_POSSIBLE, &soln,
                     &dops, &raim_removed_sids);
